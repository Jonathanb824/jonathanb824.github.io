...
var factorial = function(n) {
  ...
  //start at 5
  // first we want to check if n is less than zero
  if(n < 0){ 
...
var isEven = function(n) {
   // base cases
   // check if n is stricly equal to zero, if so we want to return true
  if (n === 0) { return true; }
...
var range = function(x, y) {
   //base case is when x is equal to y. gets from point x to y
  if( x === y || x === y - 1 || y === x - 1){
    //retuns an empty array to the callstack.
    return [];

  }
  //recursive
  //have to get everything into the array in the recursive
  if(x < y){
    // range starts at 2 and adding 1.
    // add the new range to the empty array by using concat
    return [x + 1].concat(range(x + 1, y));
  }
  if(x > y){
    //this is the reverse the range
    //range starts at 9 and subtracting 1
    return [x - 1].concat(range(x - 1, y));
...
if (exp === 0){
  ...
  // recursive case
  //return so that it can also take a negative exponent
  if(exp > 0) {
...
// JavaScript's Math object.
  // base case check if x and why is equal to zero
    // now we have to test if they are negative
...
var compareStr = function(str1, str2) {
  //compares each character of two strings
   if(str1.length === 0 && str2.length === 0){
     // return trure if both are identical 
        return true;
    } else if(str1[0] === str2[0]){
      // slice off both letter of the strings 
...
var reverseArr = function (array) {
  // create an empty array
   var reversed = [];
   // check if array.length is zero, is so return reverse
...
var rMap = function(array, callback) {
  ...
  if(array.length === 1) {
    // return the call back fucntion with the array
  ...
  // is n less than 0
   if(n < 0) {
    ...
    // else n is equal to 1
      ...
      // return 1
return 1;
}// recursive call, return nthFibo( n - 1) 
...
var capitalizeWords = function(word) {
  // create an empty array
   ...
   // if word.length is 0
    ...
    // recursive call
    result = result.concat(capitalizeWords(word.slice(1)));
    return result;
};
...
var capitalizeFirst = function(array) {
  ...
  //recursive call how do we get to an empty array
...
var letterTally = function(str, obj) {
  // create a var restult 
  let result = Array.from(arguments)[1] || {};
  // check if str length is 0
    if(str.length === 0) {
      // if so return 0 
        return result;
    }
...
var compress = function(list) {
  ...
  //if there is no duplicate, concat
 } else {
...
var minimizeZeroes = function(array) {
  // check if array.length is 0 , if so return array
...
let numToText = (str, arr = ['zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine']) => {
  if (!str.length) {
    return '';
    // returns empty string if the string input is empty from using slice
  } else if (Number(str[0])) {
    return `${arr[str[0]]}`.concat(numToText(str.slice(1)));
